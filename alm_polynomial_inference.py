...
def _plot_inference(chain, dnoisy_vector, model, errors):
    ...
    if errors is None:
        ...
        plt.errorbar(list(range(len(residuals))), residuals, errors, fmt='.')
        plt.show()
        plt.errorbar(list(range(len(residuals[::51]))), residuals[::51], errors[::51], fmt='.')
        # Compute chi-square.
...
def plot_corner(lmax, lmod, Npoly, savetag, burn_in=1000, marginal=False):
    """
